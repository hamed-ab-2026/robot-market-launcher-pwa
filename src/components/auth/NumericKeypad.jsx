...
// -----------------------------------------------------------------------
// EN: The centerpiece "Numeric Keypad" component, redesigned to match
//     the reference concept exactly: rounded-square keys (not circles),
//     soft white surface with a hairline ring, and a filled teal
//     "confirm" button in place of the bottom-end corner. The confirm
//     arrow submits manually; digits still auto-submit at full length,
//     so both interaction styles from the mock are supported.
//
//     Fully controlled: the parent (AuthPage) owns the `value` string.
//
// FA: کامپوننت اصلی "کیبورد عددی"، بازطراحی‌شده تا دقیقاً با کانسپت
//     مرجع مطابقت داشته باشد: دکمه‌های مربعیِ گردشده (نه دایره)، سطح
//     سفید نرم با یک رینگ باریک، و دکمه "تأیید" توپر سبزآبی به‌جای گوشه
//     پایین. فلش تأیید به‌صورت دستی ارسال می‌کند؛ رقم‌ها هم در تکمیل
//     طول کامل به‌صورت خودکار ارسال می‌شوند.
// -----------------------------------------------------------------------
...
export default function NumericKeypad({
  ...
  const appendDigit = useCallback(
    (digit) => {
      ...
      if (navigator.vibrate) navigator.vibrate(8); // light haptic tick, safe if unsupported
...
/** EN: A single tappable digit key — rounded square, soft ring, matches the reference. FA: یک دکمه عددی تکی، مربع گردشده، مطابق طرح مرجع. */
